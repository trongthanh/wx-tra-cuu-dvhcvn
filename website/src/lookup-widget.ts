...
function renderResultCard(label: string, address: string, wardCode: string): string {
  return `
...
async function initNewToOld(lookup: WardLookupService): Promise<void> {
  ...
  wardEl.addEventListener('change', async () => {
    ...
    let html = renderResultCard('Địa chỉ mới', formatNewAddress(newWard), newWard.ward_code);
    ...
    if (oldWards.length > 0) {
      ...
      for (const ow of oldWards) {
        html += renderResultCard('Địa chỉ cũ', formatOldAddress(ow), ow.ward_code);
      }
    } else {
...
async function initOldToNew(lookup: WardLookupService): Promise<void> {
  ...
  wardEl.addEventListener('change', async () => {
    ...
    let html = renderResultCard('Địa chỉ cũ', formatOldAddress(oldWard), oldWard.ward_code);

    html += '<div class="result-arrow">&darr;</div>';

    if (newWards.length > 0) {
      for (const nw of newWards) {
        html += renderResultCard('Địa chỉ mới', formatNewAddress(nw), nw.ward_code);
      }
    } else {
...
export async function initLookupWidget(lookup: WardLookupService): Promise<void> {
  // Tab switching
  const tabs = document.querySelectorAll<HTMLButtonElement>('.widget .tab');
  const tabContents = document.querySelectorAll<HTMLDivElement>('.widget .tab-content');
  ...
  initHelpPopovers();
  await Promise.all([initNewToOld(lookup), initOldToNew(lookup)]);
}
